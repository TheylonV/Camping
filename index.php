...
&& !empty($_POST['nomeCategoria'])) ? $_POST['nomeCategoria'] : null;
 ...
 timeZone();
  $data = dataAtual();
  $tituloDoSite = "BEM VINDO A CAMPING!";
...
if($paginaUrl === "principal"){
  ...
  cadastrarNoticia($titulo,$img,$descricao);
  ...
  $noticiasPorCategoria = listarNoticiasPorCategoria($noticia['categoria']);
}
...
include_once("./view/header-view");
  if($paginaUrl === "principal"){
    ...
    include_once("./view/noticia-view");
  }elseif($paginaUrl === "cadastrar-categoria"){
    protegerTela();
